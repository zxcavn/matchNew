...
import { CosmosCurrency, ProposalMessageType } from '@/shared/types';
...
type Props = {
  ...
  onChange: (values: CreateProposalFormValues & { type: ProposalMessageType }) => void;
  isLoading?: boolean;
  isDisabled?: boolean;
};
...
export const CreateProposalForm = ({ isLoading, isDisabled, onChange, onSubmit }: Props) => {
  ...
  >();
  const [proposalType, setProposalType] = useState<ProposalMessageType>(ProposalMessageType.TEXT);
  ...
  const onChangeForm = (values: CreateProposalFormValues) => {
    const resultValues: CreateProposalFormValues & { type: ProposalMessageType } = { ...values, type: proposalType };
          ...
          inputProps: {
            component: (
              <Select
                ...
                value={proposalType}
                onChange={e => setProposalType(e.target.value as ProposalMessageType)}
              />
            ),
          },
        },

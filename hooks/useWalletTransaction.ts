...
const useWalletTransaction = () => {
  ...
  const calculateFee = useCallback(async ({ type, options }: SendTransactionOptions): Promise<FeeState> => {
    try {
      ...
      const feeRes = { isLoading: false, fee: amount[0] as Coin };
      ...
      const feeRes = { isLoading: false, error };
